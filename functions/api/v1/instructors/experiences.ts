...
// GET /api/v1/instructors/experiences - 獲取講師的經驗分享
export async function onRequestGet(context: Context): Promise<Response> {
  ...
  try {
    ...
    if (user.userType !== 'instructor') {
      return Response.json(
        { success: false, error: { message: '只有講師可以管理經驗分享' } },
        { status: 403 }
      )
    }
    ...
    const experiences = await sql`
      SELECT 
        ...
        u.first_name,
        u.last_name,
        (SELECT COUNT(*) FROM experience_shares_likes WHERE experience_id = e.id) as likes_count,
        (SELECT COUNT(*) FROM experience_shares_comments WHERE experience_id = e.id) as comments_count
    ...
    console.error('[InstructorExperiences] 獲取經驗分享失敗:', error)
    return Response.json(
...
// PUT /api/v1/instructors/experiences - 更新經驗分享精選狀態
export async function onRequestPut(context: Context): Promise<Response> {
  ...
  try {
    ...
    console.error('[InstructorExperiences] 更新精選狀態失敗:', error)
    return Response.json(
