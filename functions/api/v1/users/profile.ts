/**
 * Users Profile API - 用戶個人資料（別名端點）
 * GET /api/v1/users/profile - 獲取當前用戶資料
 * PUT /api/v1/users/profile - 更新當前用戶資料
 * 
 * 注意：這是 /api/v1/auth/profile 的別名端點，提供相同功能
 */
...
async function handleGet(context: Context): Promise<Response> {
  ...
  try {
    ...
    handleDatabaseError(dbError, 'Get User Profile')
  }
}
...
async function handlePut(context: Context): Promise<Response> {
  ...
  try {
    ...
    handleDatabaseError(dbError, 'Update User Profile')
  }
}

export const onRequestGet = withErrorHandler(handleGet, 'Get User Profile')
export const onRequestPut = withErrorHandler(handlePut, 'Update User Profile')
