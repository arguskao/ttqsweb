...
export const getTTQSCompliance: RouteHandler = async (req: ApiRequest) => {
    try {
        ...
        const evalCounts = evaluationCheck.rows[0]
        pddroStatus.review =
            parseInt(evalCounts.reaction_count) > 0 &&
            parseInt(evalCounts.learning_count) > 0 &&
            parseInt(evalCounts.behavior_count) > 0 &&
            parseInt(evalCounts.result_count) > 0
