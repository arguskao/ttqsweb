...
export class CloudflareDatabase {
    ...
    async query<T = unknown>(sqlQuery: string, params?: unknown[]): Promise<{ rows: T[]; rowCount: number }> {
        try {
            let result: any

            if (params && params.length > 0) {
                // For parameterized queries, we need to use a different approach
                // Let's build the query manually for now
                result = await this.sql(sqlQuery, params)
            } else {
                // For simple queries without parameters
                result = await this.sql(sqlQuery)
            }
