  ...
  <div class="container mt-5">
    ...
    <div class="box">
      ...
      <div class="columns is-multiline">
        <div class="column is-one-fifth" v-for="(status, key) in complianceData.pddro_status" :key="key">
          <div class="has-text-centered">
            ...
            <p class="has-text-weight-bold mt-2">{{ getPDDROLabel(key) }}</p>
          </div>
        </div>
      </div>
      <progress 
        class="progress is-success mt-4" 
        :value="complianceData.compliance_rate" 
        max="100"
      >
          ...
          <div class="card">
            ...
            <div class="card-content">
              <div class="content">
                ...
                <p class="has-text-centered">回應數: {{ dashboardData.satisfaction?.total_responses || 0 }}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div class="card-content">
              <div class="content">
                ...
                <p class="has-text-centered">
                  ...
                  <span class="tag ml-2" :class="getPassRateClass(dashboardData.learning?.pass_rate)">
          ...
          <div class="card">
            ...
            <div class="card-content">
              <div class="content">
                ...
                <p class="has-text-centered">評估數: {{ dashboardData.behavior?.total_evaluations || 0 }}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="card">
            ...
            <div class="card-content">
              <div class="content">
                ...
                <p class="has-text-centered">
                  就業率
                  <span class="tag ml-2" :class="getEmploymentRateClass(dashboardData.employment?.employment_rate)">
                    {{ getEmploymentRateStatus(dashboardData.employment?.employment_rate) }}
                  </span>
                </p>
                <p class="has-text-centered mt-2">
                  平均薪資: NT$ {{ dashboardData.employment?.avg_salary?.toLocaleString() || 'N/A' }}
                </p>
              </div>
            </div>
          </div>
    ...
    <div class="box">
      ...
      <div class="columns">
        ...
        <div class="column is-4">
          ...
          <p class="title is-5">{{ dashboardData.improvement?.avg_effectiveness?.toFixed(2) || 'N/A' }} / 5.0</p>
        </div>
      </div>
    </div>
...
const loadData = async () => {
  try {
    // Load dashboard data
    const dashboardResponse = await axios.get('/api/v1/ttqs/dashboard')
    if (dashboardResponse.data.success) {
      dashboardData.value = dashboardResponse.data.data
    }

    // Load compliance data
    const complianceResponse = await axios.get('/api/v1/ttqs/compliance')
