...
const loadProfile = async () => {
  try {
    ...
    const response = await api.get('/api/v1/instructors/profile')
...
const submitApplication = async () => {
  ...
  try {
    ...
    await api.post('/api/v1/instructors/apply', applicationForm.value)
...
const updateProfile = async () => {
  try {
    isSaving.value = true
    await api.put('/api/v1/instructors/profile', editForm.value)
    isEditing.value = false
    alert('資料已更新')
    await loadProfile()
  } catch (error: any) {
