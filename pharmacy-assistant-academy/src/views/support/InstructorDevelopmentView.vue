...
const loadDevelopmentStatus = async () => {
  loading.value = true
  try {
    const response = await api.get('/api/v1/instructor-development/status')
...
const applyForInstructor = async () => {
  try {
    ...
    await api.post('/api/v1/instructor-development/apply', {
      certifications,
...
const logTeachingHours = async () => {
  ...
  try {
    await api.post('/api/v1/instructor-development/log-hours', {
