...
async function runMigration() {
  ...
  try {
    ...
    // 2. 檢查是否已經執行過遷移
    const existingMigrations = await pool.query('SELECT id FROM migrations WHERE id IN ($1, $2)', [
      '001_update_user_roles',
      '002_create_default_admin'
    ])

    const executedMigrations = new Set(existingMigrations.rows.map(row => row.id))
    ...
    if (!executedMigrations.has('001_update_user_roles')) {
      ...
      try {
        ...
        if (constraintCheck.rows.length > 0) {
          ...
          await pool.query(`ALTER TABLE users DROP CONSTRAINT IF EXISTS ${constraintName}`)
        }
        ...
        // 記錄遷移
        await pool.query('INSERT INTO migrations (id, filename) VALUES ($1, $2)', [
          '001_update_user_roles',
    ...
    if (!executedMigrations.has('002_create_default_admin')) {
      ...
      if (adminCount === 0) {
        ...
        await pool.query(
          ...
          VALUES ($1, $2, $3, $4, $5, $6)
        `,
          ['admin@ttqs.com', hashedPassword, 'admin', '系統', '管理員', '0900000000']
        )
